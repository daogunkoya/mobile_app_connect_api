...
class user_device_service{
    ...
    public static function log_device($request, $response){
        ...
        $user_device = mm_user_device::where('user_id',$user_id)->where('device_code',$device_code)->first();
       ...
       //  //var_dump('time',Carbon\Carbon::now()->toDateTimeString());
        ...
        if(!empty($user_device)){
            mm_user_device::where('store_id', $store_id)->where('user_id',$user_id)
    ...
    public static function location_change_email($request,$user_id, $device_name){
        ...
        $user = mm_user::where('id_user', $user_id)->select('user_email','user_handle')->first();
    ...
    public static function logout_device($request,$token){
        ...
        $user_device =  mm_user_device::where('user_id',$user_id)->where('device_access_token',$token)->first();
        $user_device =!empty($user_device)?$user_device->toArray():null;
       ...
       // var_dump('token',$token);
             ...
             if(!empty($token) && !empty($user_id)){
                    mm_user_device::where('user_id',$user_id)
