...
class transaction_service{
    ...
    public function fetch_transaction($input){
        ...
        $whereUser =  $input['user']['user_role_type'] == 1?[['mm_transaction.user_id','=', $user_id ]]:[];  
    ...
    public function date_finder($input){
...
//add a new transaction
    public function store_transaction($input){

        $user_id = $input['user']['id_user']??'';
       //var_dump($input);
        $calculate_res = $this->calculate_amount_to_send(['conversion_type'=>1,'send_amount'=>$input['amount_sent'] ]);
       // var_dump( $calculate_res);
        ...
        $bou_rate = 0;
        $sold_rate = 0;
        $receiver = optional(mm_receiver::where('id_receiver', $input['receiver_id'])
        ...
        $sender = optional(mm_sender::where('id_sender', $receiver['sender_id'])->select('sender_fname','sender_lname','sender_address' )->first())->toArray();
        if(!empty($input)){
            $receiver_phone = $receiver['receiver_phone']??$receiver['receiver_mobile'];
//var_dump($receiver['currency_id']);
           $transaction =  mm_transaction::create([
    ...
    public function update_transaction($input, $transaction_id){
        ...
        if(!empty($input) && mm_transaction::where('id_transaction', $transaction_id)->exists() ){
            //var_dump($receiver['currency_id']);
                       $transaction =  mm_transaction::where('id_transaction', $transaction_id)->update([
    ...
    public function show_amount_breakdown($input){

        $res = $this->calculate_amount_to_send($input);
