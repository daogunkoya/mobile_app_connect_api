...
use App\Models\mm_user_connect;
...
//wrap list group from array into structure of category brand & stores
        ...
        public static function get_row_user_connect($cursor){
            if(!mm_user_connect::where('id_connect',$cursor)->exists())  return response()->json(['errors'=>['cursor is not valid'], 404]);
            if($cursor)  return  mm_user_connect::where('id_item',$cursor)->value('row_number');
        ...
        public static function get_row_user($cursor){
            if(!mm_user::where('id_item',$cursor)->exists())  return response()->json(['errors'=>['cursor is not valid'], 404]);
            if($cursor)  return  mm_user::where('id_item',$cursor)->value('row_number');
    
        }

        public static function get_row_user_alert($cursor){
            if(!mm_user_alert::where('id_alert',$cursor)->exists())  return response()->json(['errors'=>['cursor is not valid'], 404]);
            if($cursor)  return  mm_user_alert::where('id_alert',$cursor)->value('row_number');
...
//get key of an item in an array--used for pagination
 ...
 //array content of store and brand
    ...
    public static function get_rate($item_group, $item_rate, $item_user_id){
        ...
        $rate_user = !empty($item_user_id)? mm_user::where('id_user',$item_user_id)->value('user_rate'):"";
...
//return overall discussion rate
    public static function get_discussion_rate($item_rate, $item_user_id, $all=null){
        ...
        $rate_user = mm_user::where('id_user',$item_user_id)->value('user_rate');
...
//delete image form s3
        ...
        public static function  initialize_store_info($user_id){
                        ...
                        $store_id = \App\Models\mm_user::where('id_user',$user_id)->value('store_id');
                        ...
                        $host_domain = mm_domain::where('store_id', $store_id)->orderBy('domain_default', 'desc')->value('domain_host');
...
//initialize session with user store_id
             if(mm_user::where('user_role_type',3)->where('user_email', $user_email)->exists()){
            $user_id = mm_user::where('user_role_type',3)->where('user_email',$user_email)->value('id_user');
            $store = Helper::initialize_store_info($user_id);
            ...
            if(mm_user::where('store_admin_type', 1)->where('store_id',$first_store_id)->exists()) return true;

            //check if user store and the environment  store  he is navigating are the same
            if($first_store_id !=  $user_store_id ) return false;


            if(!mm_user::where('store_id', $user_store_id)->where('user_email', $user_email)->exists()) return false;
