...
//wrap list group from array into structure of category brand & stores
        ...
        foreach ($list_group as $group) {
            ...
            if (!empty($result)) {
                ...
                if ($result['group_type'] == 2) {
                    $categories[] = $result['group_id'];
                }
                if ($result['group_type'] == 3) {
                    $brands[] = $result['group_id'];
                }
                if ($result['group_type'] == 4) {
                    $stores[] = $result['group_id'];
                }
                $all[] = $result;
            }
        }
    ...
    {
        if (!bd_item_connect::where('id_connect', $cursor)->exists()) {
            return response()->json(['errors' => ['cursor is not valid'], 404]);
        }
        if ($cursor) {
            return  bd_item_connect::where('id_item', $cursor)->value('row_number');
        }
    }
    ...
    public static function get_row_user_connect($cursor)
    {
        ...
        if ($cursor) {
            return  MMUser_connect::where('id_item', $cursor)->value('row_number');
        }
    }

    public static function get_row_deal($cursor)
    {
        if (!bd_item_deal::where('id_item', $cursor)->exists()) {
            return response()->json(['errors' => ['cursor is not valid'], 404]);
        }
        if ($cursor) {
            return  bd_item_deal::where('id_item', $cursor)->value('row_number');
        }
    }

    public static function get_row_discussion($cursor)
    {
        if (!bd_item_discussion::where('id_item', $cursor)->exists()) {
            return response()->json(['errors' => ['cursor is not valid'], 404]);
        }
        if ($cursor) {
            return  bd_item_discussion::where('id_item', $cursor)->value('row_number');
        }
    }
 ...
 //array content of store and brand
    public static function with_one_category($category, array $groups)
    {
        ...
        if (!empty($groups)) {
            foreach ($groups as $group) {
                if (
                    ($group['group_type'] == 2 &&  $group['group_id'] == $category) || ($group['group_type'] == 3
                            || $group['group_type'] == 4)
                ) {
                    $result[] = $group;
                }
    ...
    {
        if (is_string($previous_data)) {
            $list = json_decode($previous_data, true);
        }
...
//return overall discussion rate
   ...
   //Rebuild coupon item when user logs in
    ...
    public static function rebuild_coupon_item($user_id)
    {
        ...
        if (!empty($order_id)) {
            sqs_service::job(['type' => 'rebuild_coupon_item', 'user_id' => $user_id, 'order_id' => $order_id]);
        }
...
//initialize session with user store_id
        ...
        if ($first_store_id !=  $user_store_id) {
            return false;
        }
    ...
    public static function validate_store_request($model, $column_title, $value)
    {
        ...
        if (!empty($value) && !$model::where($column_title, $value)->where('store_id', $store_id)->exists()) {
            return response()->json(['errors' => ['store' => ["$column_title supplied never exists for this store on store $store_name"]]], 422);
        }
        return false;
    }
}
