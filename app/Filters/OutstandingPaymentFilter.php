...
class OutstandingPaymentFilter extends BaseQuery
{
    ...
    {
         // ->when($filter['status'] ?? false, fn($query) => $query->where('user_status', Status::getStatusEnumInstance($filter['status'])))  
         $this->builder->when($transactionStatus??false, fn($builder) => $builder->where('transaction_paid_status',$transactionStatus ));
        return $this->builder;
    }

    public function commission($commissionStatus)
    {
         // ->when($filter['status'] ?? false, fn($query) => $query->where('user_status', Status::getStatusEnumInstance($filter['status'])))  
         $this->builder->when($commissionStatus??false, fn($builder) => $builder->where('commission_paid_status', $commissionStatus));
        return $this->builder;
    }
